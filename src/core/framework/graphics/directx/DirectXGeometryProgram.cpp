...
void DirectXGeometryProgram::bind(void* data)
{
    ...
    _d3dContext->Map(_rendererHelper->getColorVertexBuffer().Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
    ...
    _d3dContext->Unmap(_rendererHelper->getColorVertexBuffer().Get(), 0);
    
    // Set the vertex buffer
    UINT stride = sizeof(COLOR_VERTEX);
    UINT offset = 0;
    _d3dContext->IASetVertexBuffers(0, 1, _rendererHelper->getColorVertexBuffer().GetAddressOf(), &stride, &offset);
}
