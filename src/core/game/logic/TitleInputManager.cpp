...
void TitleInputManager::update()
{
    ...
    else
    {
        for (std::vector<KeyboardEvent *>::iterator i = KEYBOARD_INPUT_MANAGER->getEvents().begin(); i != KEYBOARD_INPUT_MANAGER->getEvents().end(); ++i)
        {
            ...
            {
                case NG_KEY_A:
                    _menuState = MS_ACTIVATE_STEAM;
                    continue;
                case NG_KEY_D:
                    _menuState = MS_DEACTIVATE_STEAM;
                    continue;
                case NG_KEY_S:
                    _menuState = MS_START_SERVER;
                    continue;
                case NG_KEY_J:
                    _menuState = MS_JOIN_LOCAL_SERVER;
                    continue;
                case NG_KEY_L:
                    _menuState = MS_STEAM_REFRESH_LAN_SERVERS;
                    continue;
                case NG_KEY_I:
                    _menuState = MS_STEAM_REFRESH_INTERNET_SERVERS;
                    continue;
                case NG_KEY_ONE:
                    _menuState = MS_STEAM_JOIN_SERVER_1;
                    continue;
                case NG_KEY_TWO:
                    _menuState = MS_STEAM_JOIN_SERVER_2;
                    continue;
                case NG_KEY_THREE:
                    _menuState = MS_STEAM_JOIN_SERVER_3;
                    continue;
                case NG_KEY_FOUR:
                    _menuState = MS_STEAM_JOIN_SERVER_4;
                    continue;
                case NG_KEY_ESCAPE:
                    _menuState = MS_ESCAPE;
                    continue;
