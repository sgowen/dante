...
void CSpaceWarClient::OnLobbyCreated( LobbyCreated_t *pCallback, bool bIOFailure )
{
	...
	if ( pCallback->m_eResult == k_EResultOK )
	{
		...
		StringUtil::sprintf_safe( rgchLobbyName, "%s's lobby", SteamFriends()->GetPersonaName() );
...
void CSpaceWarClient::OnMenuSelection( LobbyMenuItem_t selection )
{
	...
	else if ( selection.m_eCommand == LobbyMenuItem_t::k_ELobbyMenuItemStartGame )
	{
		...
		LOG("Game server being created; game will start soon.\n");
	}
...
void CSpaceWarClient::OnGameWebCallback( GameWebCallback_t *callback )
{
	m_bSentWebOpen = false;
	char rgchString[256];
	StringUtil::sprintf_safe( rgchString, "User submitted following url: %s\n", callback->m_szURL );
...
void CSpaceWarClient::RunFrame()
{
	...
	case k_EClientRetrySteamConnection:
		...
		SetGameState( k_EClientConnectingToSteam );
#else
		LOG("Invalidate state k_EClientRetrySteamConnection hit on non-PS3 platform");
#endif
		break;
	case k_EClientLinkSteamAccount:
		...
		SetGameState( k_EClientConnectingToSteam );
#else
		LOG("Invalidate state k_EClientLinkSteamAccount hit on non-PS3 platform");
#endif
		break;
	case k_EClientAutoCreateAccount:
		...
		SetGameState( k_EClientConnectingToSteam );
#else
		LOG("Invalidate state k_EClientAutoCreateAccount hit on non-PS3 platform");
#endif
		break;
	...
	case k_EClientWebCallback:
		...
		if ( !m_bSentWebOpen )
		{
			...
			char szURL[MAX_PATH];
			StringUtil::sprintf_safe( szURL, "file:///%s/test.html", szCurDir );
	...
	case k_EClientMusic:
		...
		break;
	default:
		LOG("Unhandled game state in CSpaceWar::RunFrame\n");
	}
...
void CSpaceWarClient::DrawHUDText()
{
	...
	for( uint32 i=0; i<MAX_PLAYERS_PER_SERVER; ++i )
	{
		...
		if ( m_rgSteamIDPlayers[i].IsValid() )
		{
			StringUtil::sprintf_safe( rgchPlayerName, "%s", SteamFriends()->GetFriendPersonaName( playerSteamID ) );
		}
		else
		{
			StringUtil::sprintf_safe( rgchPlayerName, "Unknown Player");
		}
		...
		case 3:
			...
			break;
		default:
			LOG("DrawHUDText() needs updating for more players\n");
			break;
		}
	}
	...
	if ( m_pGameEngine->BIsSteamControllerActive( ) )
	{
		...
		{
			StringUtil::sprintf_safe( rgchHint, "No Fire action bound.");
		}
		else
		{
			StringUtil::sprintf_safe( rgchHint, "Press '%s' to Fire", rgchFireOrigin );
		}
...
void CSpaceWarClient::DrawInstructions()
{
	...
	StringUtil::sprintf_safe( rgchBuffer, "Press ESC to return to the Main Menu\n Build ID:%d", SteamApps()->GetAppBuildId() );
...
void CSpaceWarClient::DrawConnectionAttemptText()
{
	...
	if ( uSecondsLeft < 25 )
		StringUtil::sprintf_safe( rgchTimeoutString, ", timeout in %u...\n", uSecondsLeft );
	else
		StringUtil::sprintf_safe( rgchTimeoutString, "...\n");
		

	char rgchBuffer[256];
	if ( m_eGameState == k_EClientJoiningLobby )
		StringUtil::sprintf_safe( rgchBuffer, "Connecting to lobby%s", rgchTimeoutString );
	else
		StringUtil::sprintf_safe( rgchBuffer, "Connecting to server%s", rgchTimeoutString );
...
void CSpaceWarClient::UpdateRichPresenceConnectionInfo()
{
	...
	else if ( m_steamIDLobby.IsValid() )
	{
		...
		StringUtil::sprintf_safe( rgchConnectString, "+connect_lobby %llu", m_steamIDLobby.ConvertToUint64() );
	}
