...
Server::Server(bool isSteam) : m_fStateTime(0), m_fFrameStateTime(0), m_fStateTimeNoEnemies(0)
{
    ...
    if (isSteam)
    {
        NetworkManagerServer::create(new NGSteamServerHelper(STEAM_GAME_DIR, VERSION_STRING, STEAM_PRODUCT_NAME, STEAM_PRODUCT_DESC, STEAM_SERVER_PORT, STEAM_MASTER_SERVER_UPDATER_PORT, SERVER_PORT, NG_SERVER_CALLBACKS), SERVER_CALLBACKS);
    }
    else
    {
