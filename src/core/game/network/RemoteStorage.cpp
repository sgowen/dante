...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CRemoteStorage::CRemoteStorage() : m_pGameEngine()
{
	m_pRemoteStorageScreen = new CRemoteStorageScreen();
}
...
CRemoteStorageScreen::CRemoteStorageScreen( IGameEngine *pGameEngine ) : m_pGameEngine( pGameEngine )
{
	...
	if ( !m_hDisplayFont )
		LOG("RemoteStorage font was not created properly, text won't draw\n");
...
void CRemoteStorageScreen::LoadMessage()
{
	...
	if ( cubFile >= sizeof( m_rgchGreeting ) )
	{
		// ?? too big, nuke it
		char c = 0;
		LOG("RemoteStorage: File was larger than expected. . .\n");
...
void CRemoteStorageScreen::Render()
{
			...
			if ( !bQuotaExceeded )
			{
				...
				if ( ulCurrentTickCount - 150 > g_ulLastReturnKeyTick )
				{
					...
					// Do it
					{
						...
						if ( !bRet )
						{
							LOG("RemoteStorage: Failed to write file!\n");
						}
	...
	RECT rect;
	{
		...
		char rgchBuffer[256];
		StringUtil::sprintf_safe( rgchBuffer, "Num Files In Cloud: %d", m_nNumFilesInCloud );
		...
		StringUtil::sprintf_safe( rgchBuffer, "Quota: %llu bytes, %llu bytes remaining", m_ulBytesQuota, m_ulAvailableBytes );
		m_pGameEngine->BDrawString( m_hDisplayFont, rect, D3DCOLOR_ARGB( 255, 25, 200, 25 ), TEXTPOS_CENTER|TEXTPOS_VCENTER, rgchBuffer );
		...
		StringUtil::sprintf_safe( rgchBuffer, "Current Message:");
		m_pGameEngine->BDrawString( m_hDisplayFont, rect, D3DCOLOR_ARGB( 255, 25, 200, 25 ), TEXTPOS_CENTER|TEXTPOS_VCENTER, rgchBuffer );
		...
		StringUtil::sprintf_safe( rgchBuffer, "Type in a new message below:");
		m_pGameEngine->BDrawString( m_hDisplayFont, rect, D3DCOLOR_ARGB( 255, 25, 200, 25 ), TEXTPOS_CENTER|TEXTPOS_VCENTER, rgchBuffer );
		...
		StringUtil::sprintf_safe( rgchBuffer, "Hit <ENTER> to save, <ESC> to cancel");
		m_pGameEngine->BDrawString( m_hDisplayFont, rect, D3DCOLOR_ARGB( 255, 25, 200, 25 ), TEXTPOS_CENTER|TEXTPOS_VCENTER, rgchBuffer );
		...
		if ( bQuotaExceeded )
		{
			...
			StringUtil::sprintf_safe( rgchBuffer, "!! QUOTA EXCEEDED !!");
			m_pGameEngine->BDrawString( m_hDisplayFont, rect, D3DCOLOR_ARGB( 255, 25, 200, 25 ), TEXTPOS_CENTER|TEXTPOS_VCENTER, rgchBuffer );
		}
	}
}
